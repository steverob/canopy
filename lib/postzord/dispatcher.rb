...
class Postzord::Dispatcher
  ...
  # @param object [Object] The object to be sent to other Evergreen installations
  # @opt additional_subscribers [Array<Person>] Additional subscribers
  def initialize(user, object, opts={})
    @sender = user
    @object = object
    @xml = @object.to_Evergreen_xml
    @opts = opts
  ...
  def self.build(user, object, opts={})
    unless object.respond_to? :to_Evergreen_xml
      raise 'This object does not respond_to? to_Evergreen xml.  Try including Evergreen::Federated::Base into your object'
    end
  ...
  def queue_remote_delivery_job(remote_people)
    Resque.enqueue(Jobs::HttpMulti,
                   @sender.id,
                   Base64.strict_encode64(@object.to_Evergreen_xml),
  ...
  def deliver_to_local(people)
    ...
    if @object.respond_to?(:persisted?) && !@object.is_a?(Conversation)
      ...
      people.each do |person|
        Rails.logger.info("event=push route=local sender=#{@sender.Evergreen_handle} recipient=#{person.Evergreen_handle} payload_type=#{@object.class}")
  ...
  def batch_deliver_to_local(people)
    ...
    Rails.logger.info("event=push route=local sender=#{@sender.Evergreen_handle} recipients=#{ids.join(',')} payload_type=#{@object.class}")
  end

  def deliver_to_hub
    Rails.logger.debug("event=post_to_service type=pubsub sender_handle=#{@sender.Evergreen_handle}")
  ...
  def notify_users(users)
    ...
    #temp hax
    unless object_is_related_to_Evergreen_hq?
      Resque.enqueue(Jobs::NotifyLocalUsers, users.map{|u| u.id}, @object.class.to_s, @object.id, @object.author.id)
    end
  end

  def object_is_related_to_Evergreen_hq?
    (@object.author.Evergreen_handle == 'Evergreenhq@joinEvergreen.com' || (@object.respond_to?(:relayable?) && @object.parent.author.Evergreen_handle == 'Evergreenhq@joinEvergreen.com'))
  end
end
