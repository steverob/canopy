...
class Postzord::Receiver::Public < Postzord::Receiver
  ...
  # @return [Object]
  def save_object
    @object = Evergreen::Parser::from_xml(@salmon.parsed_data)
  ...
  def xml_author
    if @object.respond_to?(:relayable?)
      #this is public, so it would only be owners sending us other people comments etc
       @object.parent_author.local? ? @object.Evergreen_handle : @object.parent_Evergreen_handle
    else
      @object.Evergreen_handle
    end
  end
  ...
  def account_deletion_is_from_author
    ...
    return false if @object.Evergreen_handle != @author.Evergreen_handle
    return true
  end
