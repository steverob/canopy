...
class Postzord::Receiver::Private < Postzord::Receiver
  ...
  def receive!
    begin 
      if @sender && self.salmon.verified_for_key?(@sender.public_key)
        ...
        FEDERATION_LOGGER.info("event=receive status=abort recipient=#{@user.Evergreen_handle} sender=#{@salmon.author_id} reason='not_verified for key'")
        false
      end
    rescue => e
      #this sucks
  ...
  def parse_and_receive(xml)
    @object ||= Evergreen::Parser.from_xml(xml)
  ...
  def xml_author
    if @object.respond_to?(:relayable?)
      ...
      xml_author = @user.owns?(@object.parent) ? @object.Evergreen_handle : @object.parent_author.Evergreen_handle
      @author = Webfinger.new(@object.Evergreen_handle).fetch if @object.author
    else
      xml_author = @object.Evergreen_handle
    end
    xml_author
  end
  ...
  def relayable_without_parent?
    if @object.respond_to?(:relayable?) && @object.parent.nil?
      FEDERATION_LOGGER.error("event=receive status=abort reason='received a comment but no corresponding post' recipient=#{@user_person.Evergreen_handle} sender=#{@sender.Evergreen_handle} payload_type=#{@object.class})")
      return true
    end
  end
  ...
  def contact_required_unless_request
    unless @object.is_a?(Request) || @user.contact_for(@sender)
      FEDERATION_LOGGER.error("event=receive status=abort reason='sender not connected to recipient' recipient=#{@user_person.Evergreen_handle} sender=#{@sender.Evergreen_handle}")
      return true 
    end
  end
  ...
  def assign_sender_handle_if_request
    ...
    if @object.is_a?(Request)
      @object.sender_handle = @sender.Evergreen_handle
    end
  end
end
