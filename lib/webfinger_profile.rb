    ...
    doc.css('Link').each do |l|
      ...
      case rel
        when "http://microformats.org/profile/hcard"
          @hcard = href
        when "http://joinEvergreen.com/guid"
          @guid = href
        when "http://joinEvergreen.com/seed_location"
          @seed_location = href
      end
    end

    begin
      pubkey = text_of_attribute( doc.at('Link[rel=Evergreen-public-key]'), 'href')
