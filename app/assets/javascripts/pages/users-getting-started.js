Evergreen.Pages.UsersGettingStarted = function() {
  var self = this;
  ...
  this.subscribe("page/ready", function(evt, body) {
    ...
    $('#edit_profile').bind('ajax:complete', function(evt, xhr, status){
      ...
      var message = Evergreen.I18n.t("getting_started.hey", {'name': $("#profile_first_name").val()});
    ...
    $("#awesome_button").bind("click", function(evt){
      ...
      var confirmMessage = Evergreen.I18n.t("getting_started.no_tags");

      if(($("#as-selections-tags").find(".as-selection-item").length > 0) || confirm(confirmMessage)) {

        /* flash message prompt */
        var message = Evergreen.I18n.t("getting_started.preparing_your_stream");
        Evergreen.page.flashMessages.render({success: true, notice: message});
      } else {
        /* flash message prompt */
        var message = Evergreen.I18n.t("getting_started.alright_ill_wait");
        Evergreen.page.flashMessages.render({success: true, notice: message});
      }
    });
