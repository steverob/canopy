  ...
  updateNumber: function(dropdown, personId, number, inAspectClass){
    ...
    if (number == 0) {
      ...
      if( dropdown.closest('#publisher').length ) {
        replacement = Evergreen.I18n.t("aspect_dropdown.select_aspects");
      } else {
        replacement = Evergreen.I18n.t("aspect_dropdown.add_to_aspect");
        /* flash message prompt */
        var message = Evergreen.I18n.t("aspect_dropdown.stopped_sharing_with", {name: dropdown.data('person-short-name')});
        Evergreen.page.flashMessages.render({success: true, notice: message});
      }
    }else if (selectedAspects == allAspects) {
      replacement = Evergreen.I18n.t('aspect_dropdown.all_aspects');
    }else if (number == 1) {
      button.addClass(inAspectClass);
      replacement = dropdown.find(".selected").first().text();
      /* flash message prompt */
      if( dropdown.closest('#publisher').length == 0 ) {
        var message = Evergreen.I18n.t("aspect_dropdown.started_sharing_with", {name: dropdown.data('person-short-name')});
        Evergreen.page.flashMessages.render({success: true, notice: message});
      }
    }else {
      replacement = Evergreen.I18n.t('aspect_dropdown.toggle', { count: number.toString()})
    }
