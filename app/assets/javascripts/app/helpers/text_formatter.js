...
$(function() {
  Evergreen.url_regex = /(^|\s)\b((?:(?:https?|ftp):(?:\/{1,3})|www\.)(?:[^"<>\)\s]|\(([^\s()<>]+|(\([^\s()<>]+\)))\))+)(?=\s|$)/gi;
});
...
(function(){
  ...
  textFormatter.markdownify = function markdownify(text){
    ...
    converter.hooks.chain("preConversion", function(text) {
      ...
      text = text.replace(Evergreen.url_regex, function() {
        var url = arguments[2];
  ...
  textFormatter.mentionify = function mentionify(text, mentions) {
    var mentionRegex = /@\{([^;]+); ([^\}]+)\}/g
    return text.replace(mentionRegex, function(mentionText, fullName, EvergreenId) {
      var person = _.find(mentions, function(person){
        return (EvergreenId == person.Evergreen_id || person.handle) //jquery.mentionsInput gives us person.handle
      })
      if(person) {
