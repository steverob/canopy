...
app.models.Post.Interactions = Backbone.Model.extend({
  ...
  comment : function (text) {
    ...
    this.comments.make(text).fail(function () {
      alert(Evergreen.I18n.t("failed_to_post_message"));
    }).done(function() {
  ...
  reshare : function(){
    var interactions = this
      ...
      , flash = new Evergreen.Widgets.FlashMessages;

    reshare.save({}, {
      success : function(resp){
        flash.render({
          success: true,
          notice: Evergreen.I18n.t("reshares.successful")
        });
      },
      error: function(resp){
        flash.render({
          success: false,
          notice: Evergreen.I18n.t("reshares.duplicate")
        });
      }
    }).done(function(){
  ...
  userCanReshare : function(){
    var isReshare = this.post.get("post_type") == "Reshare"
      ...
      , userIsNotAuthor = this.post.get("author").Evergreen_id != app.currentUser.get("Evergreen_id")
      , userIsNotRootAuthor = rootExists && (isReshare ? this.post.get("root").author.Evergreen_id != app.currentUser.get("Evergreen_id") : true)
