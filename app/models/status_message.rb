...
class StatusMessage < Post
  ...
  def formatted_message(opts={})
    ...
    Evergreen::Taggable.format_tags(mentioned_message, opts.merge(:no_escape => true))
  end
  ...
  def format_mentions(text, opts = {})
    form_message = text.to_str.gsub(Mention::REGEX) do |matched_string|
      ...
      person = people.detect{ |p|
        p.Evergreen_handle == $~[2] unless p.nil?
      }
  ...
  def mentioned_people_from_string
    ...
    identifiers.empty? ? [] : Person.where(:Evergreen_handle => identifiers)
  end
