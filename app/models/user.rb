...
class User < ActiveRecord::Base
  ...
  delegate :guid, :public_key, :posts, :photos, :owns?, :image_url,
           :Evergreen_handle, :name, :public_url, :profile, :url,
  ...
  def build_post(class_name, opts={})
    ...
    opts[:Evergreen_handle] = opts[:author].Evergreen_handle

    model_class = class_name.to_s.camelize.constantize
    model_class.Evergreen_initialize(opts)
  end
  ...
  def salmon(post)
    Salmon::EncryptedSlap.create_by_user_and_activity(self, post.to_Evergreen_xml)
  end
  ...
  def set_person(person)
    ...
    person.Evergreen_handle = "#{self.username}#{User.Evergreen_id_host}"
    self.person = person
  end

  def self.Evergreen_id_host
  ...
  def seed_aspects
    ...
    if AppConfig.settings.follow_Evergreenhq?
      default_account = Webfinger.new('Evergreenhq@joinEvergreen.com').fetch
  ...
  def no_person_with_same_username
    Evergreen_id = "#{self.username}#{User.Evergreen_id_host}"
    if self.username_changed? && Person.exists?(:Evergreen_handle => Evergreen_id)
