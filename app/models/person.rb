...
class Person < ActiveRecord::Base
  ...
  before_validation :downcase_Evergreen_handle

  def downcase_Evergreen_handle
    Evergreen_handle.downcase! unless Evergreen_handle.blank?
  end
  ...
  validates :Evergreen_handle, :uniqueness => true

  scope :searchable, joins(:profile).where(:profiles => {:searchable => true})
  scope :remote, where('people.owner_id IS NULL')
  scope :local, where('people.owner_id IS NOT NULL')
  scope :for_json, select('DISTINCT people.id, people.guid, people.Evergreen_handle').includes(:profile)
  ...
  def self.search_query_string(query)
    ...
    where_clause = <<-SQL
      ...
      people.Evergreen_handle #{like_operator} ?
    SQL
  ...
  def name(opts = {})
    ...
    @name ||= Person.name_from_attrs(self.profile.first_name, self.profile.last_name, self.Evergreen_handle)
  end

  def self.name_from_attrs(first_name, last_name, Evergreen_handle)
    first_name.blank? && last_name.blank? ? Evergreen_handle : "#{first_name.to_s.strip} #{last_name.to_s.strip}".strip
  end

  def first_name
    @first_name ||= if profile.nil? || profile.first_name.nil? || profile.first_name.blank?
                self.Evergreen_handle.split('@').first
              else
  ...
  def public_url
    if self.owner
      ...
      username = self.Evergreen_handle.split("@")[0]
    end
    "#{url}public/#{username}"
  end
  ...
  def self.by_account_identifier(identifier)
    ...
    self.where(:Evergreen_handle => identifier).first
  end
  ...
  def self.create_from_webfinger(profile, hcard)
    return nil if profile.nil? || !profile.valid_Evergreen_profile?
    new_person = Person.new
    new_person.serialized_public_key = profile.public_key
    new_person.guid = profile.guid
    new_person.Evergreen_handle = profile.account
    new_person.url = profile.seed_location

    #hcard_profile = HCard.find profile.hcard.first[:href]
    Rails.logger.info("event=webfinger_marshal valid=#{new_person.valid?} target=#{new_person.Evergreen_handle}")
  ...
  def fix_profile
    Webfinger.new(self.Evergreen_handle).fetch
    self.reload
  end
end
