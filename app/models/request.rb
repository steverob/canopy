...
class Request
  ...
  # @return [Request]
  def self.Evergreen_initialize(opts = {})
    req = self.new
  ...
  def sender_handle= sender_handle
    self.sender = Person.where(:Evergreen_handle => sender_handle).first
  end

  # @note Used for XML marshalling
  # @return [String]
  def recipient_handle
    recipient.Evergreen_handle
  end
  def recipient_handle= recipient_handle
    self.recipient = Person.where(:Evergreen_handle => recipient_handle).first
  end
