...
class Reshare < Post
  ...
  def root_Evergreen_id
    self.root.author.Evergreen_handle
  end
  ...
  def after_parse
    root_author = Webfinger.new(@root_Evergreen_id).fetch
    ...
    if fetched_post
      ...
      if root_author.Evergreen_handle != fetched_post.Evergreen_handle
        raise "Evergreen ID (#{fetched_post.Evergreen_handle}) in the root does not match the Evergreen ID (#{root_author.Evergreen_handle}) specified in the reshare!"
      end
  ...
  def self.fetch_post author, guid
    ...
    Evergreen::Parser.from_xml(response.body)
  end
