...
module LayoutHelper
  ...
  def load_javascript_locales
    content_tag(:script) do
      <<-JS.html_safe
        Evergreen.I18n.loadLocale(#{get_javascript_strings_for(I18n.locale).to_json}, "#{I18n.locale}");
        Evergreen.Page = "#{params[:controller].camelcase}#{params[:action].camelcase}";
      JS
    end
  end
