...
AppConfig ||= Configurate::Settings.create do
  ...
  unless heroku? || rails_env == "test" || File.exists?(config_dir.join("Evergreen.yml"))
    $stderr.puts "FATAL: Configuration not found. Copy over Evergreen.yml.example"
    $stderr.puts "       to Evergreen.yml and edit it to your needs."
    Process.exit(1)
  end
  
  add_provider Configurate::Provider::YAML,
               config_dir.join("Evergreen.yml"),
...
if rails_env == "production"  &&
    ...
    $stderr.puts "FATAL: Evergreen doesn't know where your certificate authorities are. Please ensure they are set to a valid path in Evergreen.yml"
    Process.exit(1)
  end
end
