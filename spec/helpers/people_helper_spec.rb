...
describe PeopleHelper do
 ...
 describe "#person_image_link" do
  ...
  describe '#person_link' do
    ...
    it 'uses Evergreen handle if the person has no first or last name' do
      @person.profile.first_name = nil
      @person.profile.last_name = nil

      person_link(@person).should include @person.Evergreen_handle
    end

    it 'uses Evergreen handle if first name and first name are rails#blank?' do
  ...
  describe '#local_or_remote_person_path' do
    ...
    it "links by id if there is a period in the user's username" do
      ...
      person = @user.person
      person.Evergreen_handle = "#{@user.username}@#{AppConfig.pod_uri.authority}"
      person.save!
