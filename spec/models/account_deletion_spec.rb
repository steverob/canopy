...
describe AccountDeletion do
  it 'assigns the Evergreen_handle from the person object' do
    a = AccountDeletion.new(:person => alice.person)
    a.Evergreen_handle.should == alice.person.Evergreen_handle
  end
  ...
  describe "#perform!" do
    ...
    it 'creates a deleter' do
      AccountDeleter.should_receive(:new).with(alice.person.Evergreen_handle).and_return(stub(:perform! => true))
      @ad.perform!
    end
  ...
  describe 'serialization' do
    ...
    it 'should have a Evergreen_handle' do
      @xml.include?(alice.person.Evergreen_handle).should == true
    end
