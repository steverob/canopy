...
describe Services::Twitter do
  ...
  describe "message size limits" do
    ...
    it "should not truncate a long message with an http url" do
      long_message = " http://joinEvergreen.com/a-very-long-url-name-that-will-be-shortened.html " + @long_message_end
      long_post = stub(:text => long_message, :id => 1, :photos => [])
    ...
    it "should not truncate a long message with an https url" do
      long_message = " https://joinEvergreen.com/a-very-long-url-name-that-will-be-shortened.html " + @long_message_end
      @post.text = long_message
      answer = @service.public_message(@post, '')
      answer.should_not match /\.\.\./
    end
    ...
    it "should truncate a long message with an ftp url" do
      long_message = @long_message_start + " ftp://joinEvergreen.com/a-very-long-url-name-that-will-be-shortened.html " + @long_message_end
      long_post = stub(:text => long_message, :id => 1, :photos => [])
  ...
  describe "#profile_photo_url" do
    it 'returns the original profile photo url' do
      ...
      Twitter::Client.any_instance.should_receive(:user).with("joinEvergreen").and_return(user_stub)

      @service.nickname = "joinEvergreen"
