...
describe Reshare do
  ...
  describe "XML" do
    ...
    context 'serialization' do
      it 'serializes root_Evergreen_id' do
        @xml.should include("root_Evergreen_id")
        @xml.should include(@reshare.author.Evergreen_handle)
      end
    ...
    context 'marshalling' do
      context 'local' do
        ...
        it 'fetches the root author from root_Evergreen_id' do
      ...
      context 'remote' do
        ...
        it 'fetches the root author from root_Evergreen_id' do
          ...
          @response.stub(:body).and_return(@root_object.to_Evergreen_xml)
        ...
        context 'saving the post' do
          ...
          it 'fetches the root post from root_guid' do
            ...
            [:text, :guid, :Evergreen_handle, :type, :public].each do |attr|
          ...
          it 'verifies that the author of the post received is the same as the author in the reshare xml' do
            ...
            lambda{
              Reshare.from_xml(@xml)
            }.should raise_error /^Evergreen ID \(.+\) in the root does not match the Evergreen ID \(.+\) specified in the reshare!$/
          end
        end
      end
    end
