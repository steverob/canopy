...
describe StatusMessage do
  ...
  describe 'scopes' do
    describe '.where_person_is_mentioned' do
      it 'returns status messages where the given person is mentioned' do
        @bo = bob.person
        @test_string = "@{Daniel; #{@bo.Evergreen_handle}} can mention people like Raph"
  ...
  describe '#Evergreen_handle=' do
    it 'sets #author' do
      ...
      post.Evergreen_handle = person.Evergreen_handle
...
#    n.valid?.should be_false
  ...
  describe 'mentions' do
    before do
      ...
      @test_string = <<-STR
@{Raphael; #{@people[0].Evergreen_handle}} can mention people like Raphael @{Ilya; #{@people[1].Evergreen_handle}}
can mention people like Raphaellike Raphael @{Daniel; #{@people[2].Evergreen_handle}} can mention people like Raph
STR
    ...
    describe '#format_mentions' do
      ...
      context 'with :plain_text option' do
        it 'removes the mention syntax and displays the unformatted name' do
          status  = FactoryGirl.build(:status_message, :text => "@{Barack Obama; barak@joinEvergreen.com } is so cool @{Barack Obama; barak@joinEvergreen.com } ")
...
.values
  ...
  describe "XML" do
    ...
    it 'serializes the author address' do
      @xml.should include(@user.person.Evergreen_handle)
    end
    ...
    describe '.from_xml' do
      ...
      it 'marshals the Evergreen_handle' do
        @marshalled.Evergreen_handle.should == @message.Evergreen_handle
      end
    end
  end
  ...
  describe 'oembed' do
    before do
      ...
      @message_text = "#{@youtube_url} is so cool. so is this link -> https://joinEvergreen.com"
    end
