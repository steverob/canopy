...
describe Person do
  ...
  describe '#Evergreen_handle' do
    context 'local people' do
      it 'uses the pod config url to set the Evergreen_handle' do
        new_person = User.build(:username => "foo123", :email => "foo123@example.com", :password => "password", :password_confirmation => "password").person
        new_person.Evergreen_handle.should == "foo123#{User.Evergreen_id_host}"
      end
      ...
      it 'does not include www if it is set in app config' do
        ...
        new_person = User.build(:username => "foo123", :email => "foo123@example.com", :password => "password", :password_confirmation => "password").person
        new_person.Evergreen_handle.should == "foo123@foobar.com"
      end
    end

    context 'remote people' do
      it 'stores the Evergreen_handle in the database' do
        @person.Evergreen_handle.include?(AppConfig.pod_uri.host).should be false
      end
    end

    describe 'validation' do
      it 'is unique' do
        person_two = FactoryGirl.build(:person, :Evergreen_handle => @person.Evergreen_handle)
        person_two.should_not be_valid
      end

      it 'is case insensitive' do
        person_two = FactoryGirl.build(:person, :Evergreen_handle => @person.Evergreen_handle.upcase)
        person_two.should_not be_valid
      end
    end
  end
  ...
  context '.name_from_attrs' do
    ...
    context 'with only first name' do
      it 'should return their first name for name' do
        Person.name_from_attrs(@profile.first_name, nil, @profile.Evergreen_handle).should == @profile.first_name.strip
      end
    end

    context 'with only last name' do
      it 'should return their last name for name' do
        Person.name_from_attrs(nil, @profile.last_name, @profile.Evergreen_handle).should == @profile.last_name.strip
      end
    end

    context 'with both first and last name' do
      it 'should return their composed name for name' do
        Person.name_from_attrs(@profile.first_name, @profile.last_name, @profile.Evergreen_handle).should == "#{@profile.first_name.strip} #{@profile.last_name.strip}"
      end
    end

    context 'without first nor last name' do
      it 'should display their Evergreen handle' do
        Person.name_from_attrs(nil, nil, @profile.Evergreen_handle).should == @profile.Evergreen_handle
      end
    end
  end
  ...
  describe '#name' do
    it 'calls Person.name_from_attrs' do
      ...
      Person.should_receive(:name_from_attrs).with(profile.first_name, profile.last_name, profile.person.Evergreen_handle)
      alice.name
    end
  ...
  describe '.search' do
    ...
    it 'returns results for Evergreen handles' do
      people = Person.search(@robert_grimm.Evergreen_handle, @user)
  ...
  context 'people finders for webfinger' do
    ...
    describe '.by_account_identifier' do
      it 'should find a local users person' do
        p = Person.by_account_identifier(user.Evergreen_handle)
        p.should == user.person
      end

      it 'should find remote users person' do
        p = Person.by_account_identifier(person.Evergreen_handle)
        p.should == person
      end

      it 'should downcase and strip the Evergreen_handle' do
        dh_upper = "    " + user.Evergreen_handle.upcase + "   "
      ...
      it "finds a local person with a mixed-case username" do
        ...
        person = Person.by_account_identifier(user.person.Evergreen_handle)
        person.should == user.person
      end

      it "is case insensitive" do
        user1 = FactoryGirl.create(:user, :username => "SaMaNtHa")
        person = Person.by_account_identifier(user1.person.Evergreen_handle.upcase)
        person.should == user1.person
      end

      it 'should only find people who are exact matches (1/2)' do
        user = FactoryGirl.create(:user, :username => "SaMaNtHa")
        person = FactoryGirl.create(:person, :Evergreen_handle => "tomtom@tom.joinEvergreen.com")
        user.person.Evergreen_handle = "tom@tom.joinEvergreen.com"
        user.person.save
        Person.by_account_identifier("tom@tom.joinEvergreen.com").Evergreen_handle.should == "tom@tom.joinEvergreen.com"
      end

      it 'should only find people who are exact matches (2/2)' do
        person = FactoryGirl.create(:person, :Evergreen_handle => "tomtom@tom.joinEvergreen.com")
        person1 = FactoryGirl.create(:person, :Evergreen_handle => "tom@tom.joinEvergreen.comm")
        f = Person.by_account_identifier("tom@tom.joinEvergreen.com")
        f.should be nil
      end
    ...
    describe '.local_by_account_identifier' do
      it 'should find local users people' do
        p = Person.local_by_account_identifier(user.Evergreen_handle)
        p.should == user.person
      end

      it 'should not find a remote person' do
        p = Person.local_by_account_identifier(@person.Evergreen_handle)
        p.should be nil
      end

      it 'should call .by_account_identifier' do
        Person.should_receive(:by_account_identifier)
        Person.local_by_account_identifier(@person.Evergreen_handle)
      end
    end
  end
  ...
  describe '#as_json' do
    it 'returns a hash representation of a person' do
      @person.as_json.should == {
        ...
        :handle => @person.Evergreen_handle,
