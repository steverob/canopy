...
describe User do
  ...
  describe "validation" do
    ...
    describe "of username" do
      ...
      it 'requires uniqueness also amount Person objects with Evergreen handle' do
        p = FactoryGirl.create(:person, :Evergreen_handle => "jimmy#{User.Evergreen_id_host}")
  ...
  describe '#update_profile' do
    ...
    context 'passing in a photo' do
      before do
        ...
        @photo = Photo.Evergreen_initialize(:author => alice.person, :user_file => image)
        @photo.save!
        @params = {:photo => @photo}
      end
  ...
  describe "#seed_aspects" do
    ...
    describe "Evergreenhq sharing" do
      let(:user) {
        FactoryGirl.create(:user)
      }

      before(:each) do
        @old_followhq_value = AppConfig.settings.follow_Evergreenhq?
      end

      after(:each) do
        AppConfig.settings.follow_Evergreenhq = @old_followhq_value
      end

      context "with sharing with Evergreenhq enabled" do
        it "should start sharing with the Evergreenhq account" do
          AppConfig.settings.follow_Evergreenhq = true
      ...
      context "with sharing with Evergreenhq enabled" do
        it "should not start sharing with the Evergreenhq account" do
          AppConfig.settings.follow_Evergreenhq = false
