...
describe Jobs::FetchProfilePhoto do
  ...
  it 'saves the profile image' do
    ...
    Photo.should_receive(:Evergreen_initialize).with(hash_including(:author => @user.person, :image_url => @url, :pending => true)).and_return(@photo_stub)
  ...
  context "service does not have a profile_photo_url" do
    it "does nothing without fallback" do
      ...
      Photo.should_not_receive(:Evergreen_initialize)
    ...
    it "fetches fallback if it's provided" do
      @photo_stub.should_receive(:save!).and_return(true)
      @service.stub!(:profile_photo_url).and_return(nil)
      Photo.should_receive(:Evergreen_initialize).with(hash_including(:author => @user.person, :image_url => "https://service.com/fallback_lowres.jpg", :pending => true)).and_return(@photo_stub)
  ...
  it 'updates the profile' do 
    ...
    Photo.should_receive(:Evergreen_initialize).and_return(@photo_stub)
