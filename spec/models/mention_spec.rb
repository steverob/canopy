...
describe Mention do
  describe "#notify_recipient" do
    ...
    it 'notifies the person being mentioned' do
      sm = @user.build_post(:status_message, :text => "hi @{#{bob.name}; #{bob.Evergreen_handle}}", :to => @user.aspects.first)
    ...
    it 'should not notify a user if they do not see the message' do
      ...
      sm2 = bob.build_post(:status_message, :text => "stuff @{#{alice.name}; #{alice.Evergreen_handle}}", :to => bob.aspects.first)
