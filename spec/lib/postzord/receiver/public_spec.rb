...
describe Postzord::Receiver::Public do
  ...
  context 'round trips works with' do
    it 'a comment' do
      ...
      xml = Salmon::Slap.create_by_user_and_activity(bob, comment.to_Evergreen_xml).xml_for(nil)
      bob.destroy
      comment.destroy
      expect{
  ...
  describe '#receive_relayable' do 
    before do
      ...
      @comment.save
      created_salmon = Salmon::Slap.create_by_user_and_activity(alice, @comment.to_Evergreen_xml)
      xml = created_salmon.xml_for(nil)
      @comment.delete
      @receiver = Postzord::Receiver::Public.new(xml)
    end
