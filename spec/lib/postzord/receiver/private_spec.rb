  ...
  describe '#receive!' do
    ...
    context 'returns the sent object' do
      it 'returns the received object on success' do
        @zord.receive!
        @zord.instance_variable_get(:@object).should respond_to(:to_Evergreen_xml)
      end
    end

    it 'parses the salmon object' do
      Evergreen::Parser.should_receive(:from_xml).with(@salmon.parsed_data).and_return(@alices_post)
      @zord.receive!
    end
  end
