...
describe Postzord::Dispatcher do
  ...
  describe '.initialize' do
    it 'sets @sender, @object, @xml' do
      ...
      zord.xml.should == @sm.to_Evergreen_xml
    end
    ...
    it 'raises and gives you a helpful message if the object can not federate' do
      ...
      }.to raise_error /Evergreen::Federated::Base/
    end
  end
  ...
  context 'instance methods' do
    ...
    describe '#deliver_to_services' do
      ...
      it 'does not push to hub for non-public posts' do
       ...
       mailman = Postzord::Dispatcher.build(alice, @sm, :url => "http://joinEvergreen.com/p/123")
      ...
      it 'only pushes to specified services' do
       ...
       alice.services << @s2
       mailman = Postzord::Dispatcher.build(alice, FactoryGirl.create(:status_message), :url => "http://joinEvergreen.com/p/123", :services => [@s1])
      ...
      it 'does not push to services if none are specified' do
       mailman = Postzord::Dispatcher.build(alice, FactoryGirl.create(:status_message), :url => "http://joinEvergreen.com/p/123")
