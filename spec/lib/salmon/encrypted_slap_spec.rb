...
describe Salmon::EncryptedSlap do
  before do
    ...
    @created_salmon = Salmon::EncryptedSlap.create_by_user_and_activity(alice, @post.to_Evergreen_xml)
  end
  ...
  describe '#create' do
    it 'makes the data in the signature encrypted with that key' do
      ...
      alice.aes_decrypt(decoded_string, key_hash).should == @post.to_Evergreen_xml
    end
  ...
  describe "#process_header" do
    ...
    it 'sets the author id' do
      @new_slap.author_id.should == alice.Evergreen_handle
    end
  ...
  describe '#xml_for' do
    ...
    context "encrypted header" do
      ...
      it 'contains the author id' do
        @dh_doc.search('author_id').map(&:text).should == [alice.Evergreen_handle]
      end
    end
  end
end
