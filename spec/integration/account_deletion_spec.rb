...
describe 'deleteing your account' do
  context "user" do
    before do
      @bob2 = bob
      @person = @bob2.person
      @alices_post = alice.post(:status_message, :text => "@{@bob2 Grimn; #{@bob2.person.Evergreen_handle}} you are silly", :to => alice.aspects.find_by_name('generic'))
      ...
      AccountDeleter.new(@bob2.person.Evergreen_handle).perform!
      @bob2.reload
    end
  ...
  context 'remote person' do
    before do
      ...
      AccountDeleter.new(@person.Evergreen_handle).perform!
      @person.reload
    end
