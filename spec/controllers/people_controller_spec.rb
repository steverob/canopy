...
describe PeopleController do
  ...
  describe '#index (search)' do
    ...
    describe 'via html' do
      context 'query is a Evergreen ID' do
        before do
          @unsearchable_eugene = FactoryGirl.create(:person, :Evergreen_handle => "eugene@example.org",
      ...
      context 'query is not a tag or a Evergreen ID' do
  ...
  describe '#hovercard' do
    ...
    it 'returns json with profile stuff' do
      ...
      JSON.parse( response.body )['handle'].should == @hover_test.Evergreen_handle
    end
  end
  ...
  describe '#refresh_search ' do
    ...
    describe 'via json' do
      ...
      it 'returns with a found name' do
        get :refresh_search, :q => @korth.Evergreen_handle
  ...
  describe '#Evergreen_id?' do
    it 'returns true for pods on urls' do
      @controller.send(:Evergreen_id?, "ilya_123@pod.geraspora.de").should be_true
    end

    it 'returns true for pods on urls with port' do
      @controller.send(:Evergreen_id?, "ilya_123@pod.geraspora.de:12314").should be_true
    end

    it 'returns true for pods on localhost' do
      @controller.send(:Evergreen_id?, "ilya_123@localhost").should be_true
    end

    it 'returns true for pods on localhost and port' do
      @controller.send(:Evergreen_id?, "ilya_123@localhost:1234").should be_true
    end

    it 'returns true for pods on ip' do
      @controller.send(:Evergreen_id?, "ilya_123@1.1.1.1").should be_true
    end

    it 'returns true for pods on ip and port' do
      @controller.send(:Evergreen_id?, "ilya_123@1.2.3.4:1234").should be_true
    end

    it 'returns false for pods on with invalid url characters' do
      @controller.send(:Evergreen_id?, "ilya_123@join_Evergreen.com").should be_false
    end

    it 'returns false for invalid usernames' do
      @controller.send(:Evergreen_id?, "ilya_2%3@joinEvergreen.com").should be_false
    end
  end
end
