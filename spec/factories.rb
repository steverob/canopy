#   Copyright (c) 2010-2011, Evergreen In  This file is
...
FactoryGirl.define do
  ...
  factory :person do
    sequence(:Evergreen_handle) { |n| "bob-person-#{n}#{r_str}@example.net" }
  ...
  factory :account_deletion do
    ...
    after(:build) do |delete|
      delete.Evergreen_handle = delete.person.Evergreen_handle
    end
  end
  ...
  factory :user do
    ...
    after(:build) do |u|
      u.person = FactoryGirl.build(:person, :profile => FactoryGirl.build(:profile),
                                  ...
                                  :Evergreen_handle => "#{u.username}#{User.Evergreen_id_host}")
    end
    after(:create) do |u|
      u.person.save
  ...
  factory(:status_message) do
    ...
    association :author, :factory => :person
    after(:build) do |sm|
      sm.Evergreen_handle = sm.author.Evergreen_handle
    end
  end
